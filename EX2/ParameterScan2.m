...
for i = 1:nsimul
  output{i} = ['application1/simulations/',paramstr, '=', num2str(param(i)), '.out'];
  % Execution du programme en lui envoyant la valeur a scanner en argument
  cmd = sprintf('%s%s %s E=6e4 %s=%.15g output=%s', repertoire, executable, input, paramstr, param(i), output{i});
  disp(cmd)
  system(cmd);
end
%% Analyse %%
...
print('application1/etudeConv1_2', '-depsc');
...
print('application1/etudeConv1_1', '-depsc');
%set(gcf, 'Units', 'inches');
%screenposition = get(gcf, 'Position');
%set(gcf, 'PaperPosition', [0 0 screenposition(3:4)],
%         'PaperSize', [screenposition(3:4)]);
%print('application1/etudeConv4', '-dpdf');
